...
function setActivitiesAsDefault() {
    ...
    if(document.location.pathname.toLowerCase() == '/v4/pages/search.aspx' && exl_grp != 'act')
    {
       document.location = document.location + '&exl_grp=act';
    }
}
...
function selectLanguage(){
    ...
    langOptions.forEach(function (item, index) {
        ...
        $(".language-select").append(`<option value="${item.id}" ${selectedLang}>${item.label}</option>`);
    });
...
function changeLanguage(){
    ...
    window.location.href = `${page}?lang=${selectedLang}`;
}
...
function isLandingPage() {
    return document.location.host === 'visithachinohe.com' || document.location.host === 'hadi.txi.co.id';
}
...
function getTopRightMenu() {
    ...
    const _links = getLanguage().substring(0,2).toLowerCase() == 'ja'
...
function get_header() {
    ...
    const logoImage = getLanguage().substring(0,2).toLowerCase() == 'en'
            ...
            <nav class="header-nav--main" role="navigation" aria-label="メインメニュー">
                <ul>
                    ...
                    <li class="header-nav-item popup-hidden"><a href="${siteLanguage}/shopping" class="LT-42">Bring Hachinohe Home</a></li>
                       <li class="header-nav-item popup-hidden"><a href="${siteLanguage}/activities"><span  class="LT-91">Book / Purchase</a></li>
                </ul>
            </nav>
        </div>
